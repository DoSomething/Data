...
# (y should be public.users data or any other membership data).
...
```{r}
# this code will soon become obsolete as Jen is leading the sprint to combine for all columns 
reg_status <- predicted(users1, turbo1)
...
table(reg_status$voter_reg_status)
```
...
df_selfreport$voter_reg_status <- # factor to only include the selected levels 
  factor(df_selfreport$voter_reg_status, 
       levels = c("confirmed", "unregistered"),
       labels = c("confirmed", "unregistered"))
...
table_rf <- table(test_transformed$voter_reg_status, yhat_rf)

# simple accuracy 
sum(diag(table_rf)) / sum(table_rf) 

# AUC (true positive rate vs. false positive rate)
test1 <- test_transformed
test1$voter_reg_status <- ifelse(test1$voter_reg_status == "confirmed", 1, 0)
yhat_rf1 <- yhat_rf
yhat_rf1 <- ifelse(yhat_rf1 == "confirmed", 1, 0)
roc_obj <- roc(test1$voter_reg_status, yhat_rf1)
auc(roc_obj)
```
