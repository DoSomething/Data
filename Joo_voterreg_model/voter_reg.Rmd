...
packages.used <- c("lubridate", "reshape2", "caret", "ggplot2", "MLmetrics",
...
# MUST replace NAs in non-numeric columns (except voter_reg_status) with "unknown" for model to run
...
Ensures that some features do not dominate the algorithm AND and the features are unit-independent. 
...
saveRDS(data_ready_predict, "Rdata/data_ready_predict.rds")
```
...
```{r}
by_grp <- up_training %>% group_by(voter_reg_status)
train_transformed_sample <- sample_n(by_grp, 10000)

rm(master, by_grp, data_ready_predict, in_train, training, up_training)
```
...
# Choose model and save model for future predictions 
...
data_ready_predict <- readRDS("Rdata/data_ready_predict.rds") 

# create a table that has northstar_id, voter_reg_status, untransformed predictors, and predictions 
voter_reg_prediction <- voter_reg_predict(data_ready_predict)
```
...
```{r}
# I have the final prediction, it now needs to go into Quasar 
channel <- pgConnect()

if(dbExistsTable(channel, c("public", "voter_reg_predictions"))) {

  q <- "truncate public.voter_reg_predictions"
  runQuery(q)
...
# Adding new DoSomething into data and making predictions for them 

```{r}
best_model <- load(TKTKTKTKTKTKTTK)
```
...
```{r}
# pie chart of gender breakdown - fix 
...
```{r}
# bar chart of gender breakdown by voter reg status 
voter_reg_gender <- 
  master %>% 
...
# remove those with dosomething.org email address? 
mean(master$total_action) # average number of actions 

# cause areas ?
master %>% 
  select(starts_with("cause_")) %>% 
  mutate_at(vars(1:4), funs(replace(., is.na(.), 0))) %>%
  mutate(total = )
```
