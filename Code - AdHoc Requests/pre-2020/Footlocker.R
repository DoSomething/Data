...
footlocker_quantify<-footlocker_markets%>%
    ...
    mutate(
      ...
      badminton=ifelse(participation %like% 'badmin',1,0),
      boxing=ifelse(participation %like% 'box',1,0),
      rowing=ifelse(participation %like% 'row',1,0),
      karate=ifelse(participation %like% 'karate',1,0),
      rifle=ifelse(participation %like% 'rifl',1,0),
      taekwondo=ifelse(participation %like% 'tae',1,0),
      wrestling=ifelse(participation %like% 'wrest',1,0),
      rugby=ifelse(participation %like% 'rugb',1,0),
      wheelchair=ifelse(participation %like% 'wheel',1,0),
...
Footlocker_totalscore_final <-Footlocker_totalscore%>%
  dplyr::select(1:7,9,11:19,24:55,71,91:117,20,21,61,23,59,22,60,8,57:58,10,62,56,90,118:120)

# Footlocker_totalscore_final<-str_ucfirst('first_name')

#Export to csv
write.csv(Footlocker_totalscore_final, file = "Footlocker Total Scores.csv")
