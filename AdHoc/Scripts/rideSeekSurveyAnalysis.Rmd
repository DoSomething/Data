...
metNames <- c('Seatbelt.Back','Seatbelt.Front','Seatbelt.Drive',
...
```{r echo=FALSE}
pivotBars <- function(group='group', pivot=NULL, title=NULL) {
  if (is.null(pivot)) {
    dat %>% 
      group_by_(group) %>% 
      summarise(
      ...
      melt(id.var=c(group)) %>%
      ggplot(.,aes(x=variable, y=value, fill=get(group))) +
  ...
  dat %>% 
    ...
    geom_bar(stat='identity', position='dodge') +
    geom_text(aes(label=round(value, 3)), position = position_dodge(width = 1), size=2) +
    coord_flip() + ggtitle(paste0('Pivoted by ',title)) +
...
getProportions <- function(dat, outcome, group=NULL) {
  
  split <- strsplit(outcome, '_')[[1]]
  proportions <-
    dat %>% 
    group_by_(group, outcome) %>% 
    summarise(count = n()) %>% 
  ...
  if (grepl('seatbelt',outcome)) {
    ggplot(proportions, aes(x=outcome, y=Proportion, fill=get(group))) + 
...
for (i in 1:length(histOutcomes)) {
  getProportions(dat, histOutcomes[i], group='Group') %>% 
    print()
}
```
...
* Overall, men are less less responsive to the campaign across all metrics.
...
for (i in 1:length(controls)) {
  pivotBars(controls[i], titles[i],group = 'Group') %>% 
    print()
}
```
