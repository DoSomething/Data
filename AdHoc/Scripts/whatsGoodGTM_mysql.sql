SELECT 
	...
	actions.active_april,
	actions.actions_april
FROM 
	(SELECT 
		c.northstar_id,
...
LEFT JOIN 
	...
	WHERE c1.signup_created_at >= '2018-01-01'
...
LEFT JOIN 
	(SELECT
		...
		max(CASE WHEN mel.`timestamp` >= '2018-04-01' AND mel.`timestamp` < '2018-04-07' THEN 1 ELSE 0 END) AS active_april,
		sum(CASE WHEN mel.`timestamp` >= '2018-04-01' AND mel.`timestamp` < '2018-04-07' THEN 1 ELSE 0 END) AS actions_april,
		sum(CASE WHEN mel.action_type IN ('site_access','site_login') THEN 1 ELSE 0 END) AS site_visits,
		count(*) AS total_actions
	FROM member_event_log mel
	WHERE mel.`timestamp` >= '2018-01-01'
