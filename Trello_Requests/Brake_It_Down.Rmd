...
####34% said their friend would want to drive a convertible, 31% said an electric car####
...
all_bid_readguide <-all_bid_readguide %>%
  ...
  mutate(survey_submit = as.Date(as.numeric(as.Date(substr(Submit.Date..UTC., 1, 10))), origin='1970-01-01'),
           ...
           #   as.numeric(age),
           # age_over18 =
           #   if_else(age_numeric>18,"Older than 18","18 and younger"),
...
```{r, include=FALSE}
#Bar chart
feelings_chart <- ggplot(feelings_pct, aes(x=feeling, y=pct)) + 
...
pivotBars <- function(expid='group', pivot=NULL, title=NULL) {
  if (is.null(pivot)) {
    ...
    all_bid_readguide %>% 
      ...
      summarise(
        Reduces_crash = mean(perception_reduces_crash),
        Five_mph_acceptable= mean(perception_5mph_acceptable),
        Fifteen_mph_notacceptable = meant(perception_15mph_notacceptable),
        Dangers = mean(danger)
      ) %>%
...
pivotBars_dems <- function(expid='group', pivot=NULL, title=NULL) {
  if (is.null(pivot)) {
    all_bid_readguide %>% 
      group_by_(expid) %>% 
      summarise(
        ...
        `Unacceptable to exceed limits by more than 15mph` = mean(perception_15mph_notacceptable),
...
####Participants from rural/suburban areas and drivers/passengers were also more responsive on these two areas compared to other respective young people in the control group####

```{r}
#chart
controls <- c('region', 'transportation')
titles <- c('Region', 'Transportation Type')
for (i in 1:length(controls)) {
  pivotBars_dems(controls[i], titles[i],expid = 'group') %>% 
    print()
}
```
