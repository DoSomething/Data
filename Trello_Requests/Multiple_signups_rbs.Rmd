...
email_activity <- active_last6month%>%
  mutate(multiple_signup=ifelse(total_signups>1,'2+ signups','1 or less signup'),
         ...
         hours_between_signup = abs(difftime(last_signup, first_signup, units = "hours")),
         hours_between_rb = abs(difftime(last_rb, first_rb, units = "hours")),
         days_between_signup = abs(difftime(last_signup, first_signup, units = "days")),
         days_between_rb = abs(difftime(last_rb, first_rb, units = "days")),
...
* *Only 28% of members who signed up in the last year signed up for 2+ campaigns*
```{r}
#Pct of members with 2+ signups in the last year
count(email_activity, multiple_signup, sort = TRUE)%>%mutate(pct=n/sum(n)*100)
```

* *Looking at only those with 2+ signups, the majority (75.2%) signed up for their second campaign over a week after their first campaign. *
* *15.8% signed up for more than one campaign on the same day.*
...
```
* *For multiple signups on the same day, the median time between the first and last signup time for multiple signups was 0.15hrs.*
```{r}
...
* *The majority of members who had multiple signups in a day, only signed up for **2** campaigns.*
```{r}
...
ggplot(email_activity, aes(x=total_rbs)) +
  geom_bar() + geom_text(stat='count',aes(label=..count..),vjust=-1) + xlim(0,10) + ylim(c(0,36000)) +
  labs(x='Count of photo rbs/member in the last year', title = 'Total photo rbs per member in the last year')
```

* *Looking at those who have multiple rbs, 30% submitted photo rbs on the same day*
```{r}
...
ggplot(multiple_rbs, aes(x=time_multiple_rbs)) + geom_bar() +
  geom_text(stat='count',aes(label=..count..),vjust=-1) + ylim(c(0,11000)) +
...
* *The majority of members who had multiple photo rbs in a day, only reported back **2** times in a day.*
```{r}
#Distribution of members with 2+ signups on the same day
