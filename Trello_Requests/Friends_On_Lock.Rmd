...
count(fol_all, sleepy_reason, sort = TRUE)%>% filter(sleepy_reason!=is.na(sleepy_reason))%>%mutate(pct=round(n/sum(n)*100,2))
```
...
```{r}
count(fol_all, freq_alcohol, sort = TRUE)%>% filter(freq_alcohol!=is.na(freq_alcohol))%>%mutate(pct=round(n/sum(n)*100,2))
```
...
```{r}
count(fol_all, freq_sleepy, sort = TRUE)%>% filter(freq_sleepy!=is.na(freq_sleepy))%>%mutate(pct=round(n/sum(n)*100,2))
```
...
feelings_merged <-cbind(feelings_ct,feelings_names)%>%
    mutate(
      group=ifelse(grepl('alcohol',feeling),"drugs/alcohol","sleepy")
      )%>%
  group_by(group)%>%
...
pivotBars <- function(expid='group', pivot=NULL, title=NULL) {
  if (is.null(pivot)) {
    fol_all %>% 
      group_by_(expid) %>% 
      summarise(
        `Dangerous to drive under the influence` = round(mean(dangerous_alcohol),2),
        `Dangerous to drive while sleepy`= round(mean(dangerous_sleepy),2),
        `Driving while sleepy should be considered impaired driving` = round(mean(agree_impaired),2),
        `Comfortable talking about driving while under the influence` = round(mean(agree_convo_alcohol),2),
        `Comfortable talking about driving while sleepy` = round(mean(agree_convo_sleepy),2)
      ) %>%
      melt(id.var=c(expid)) %>%
    ...
    fol_all %>% 
      ...
      summarise(
        ...
        Convo_alcohol = round(mean(agree_convo_alcohol),2),
        Convo_sleepy = round(mean(agree_convo_sleepy),2)
      ) %>%
...
#chart
controls <- c('region', 'transportation_cat', 'gender')
...
###The majority of young people were likely to take the second most severe intervention of offering to drive or calling a rideshare###

####We saw no significant improvement on likelihood to intervene on a driver under the influence using the most severe intervention (p>0.05)####

####We saw a small significant improvement on likelihood to intervene on a driver who was sleepy/drosy using the most severe intervention (p<0.05)####
...
```{r, include=FALSE}
#Bar charts
intervened_chart <- ggplot(intervened, aes(x=attempted_intervention, y=pct, group = group, fill=group)) + 
                        geom_bar(stat="identity", position = "dodge", colour="grey") +
                        geom_text(aes(label=scales::percent(pct)), 
                                  vjust=-1, position = position_dodge(width=0.9),  size=3) + 
                        labs(x= 'Tried to convince them to pull over', y = 'Percentage') + 
                        scale_y_continuous(labels = scales::percent_format(accuracy = 1), 
                                           limits = c(0, 0.7)) + 
                        theme(axis.text.x = element_text(angle = 0, hjust = 1), 
                              ...
                              axis.title.x = element_blank(),
                              axis.title.y = element_blank()) + 
                        ggtitle("Did you attempt in any way to stop the driver from driving??") +
                        scale_fill_manual(values=c('#000000',"#23b7fbff"))
```
...
```{r}
count(fol_all, likelihood_alcohol, sort = TRUE)%>%filter(likelihood_alcohol!=is.na(likelihood_alcohol))%>%mutate(pct=round(n/sum(n)*100,2))
```
...
```{r}
count(fol_all, likelihood_sleepy, sort = TRUE)%>%filter(likelihood_sleepy!=is.na(likelihood_sleepy))%>%mutate(pct=round(n/sum(n)*100,2))
```
